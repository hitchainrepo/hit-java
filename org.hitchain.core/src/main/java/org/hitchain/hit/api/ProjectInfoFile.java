...
public class ProjectInfoFile implements Serializable {
    ...
    public static void main(String[] args) throws Exception {
        ...
        KeyPair rootKeyRsa = RSAHelper.generateKey();
        ...
        KeyPair helloKeyRsa = RSAHelper.generateKey();
        ...
        info.repoPriKey = Hex.toHexString(RSAHelper.encrypt(repoKeyPair.getPrivKeyBytes(), rootKeyRsa.getPublic()));
        ...
        byte[] bs = ECCHelper.publicEncrypt(genSignedContent.getBytes(),
                ECCHelper.getPublicKeyFromEthereumPublicKeyHex(info.repoPubKey));

        byte[] repoPriKeyBs = RSAHelper.decrypt(Hex.decode(info.repoPriKey), rootKeyRsa.getPrivate());
        byte[] decrypt = ECCHelper.privateDecrypt(bs,
                ECCHelper.getPrivateKeyFromEthereumHex(Hex.toHexString(repoPriKeyBs)));
    ...
    public String genSignedContent(String ownerPriKey) {
        ...
        byte[] sign = RSAHelper.sign(ByteHelper.utf8(content), RSAHelper.getPrivateKeyFromHex(ownerPriKey));
    ...
    public boolean verify(ProjectInfoFile old) {
        ...
        return RSAHelper.verify(ByteHelper.utf8(getContent()), Hex.decode(getSign()),
                RSAHelper.getPublicKeyFromHex(getOwnerPubKeyRsa()));
    }
    ...
    public void addMemberPrivate(String member, String memberPubKeyRsa, String memberAddressEcc,
        ...
        byte[] decrypt = RSAHelper.decrypt(Hex.decode(getRepoPriKey()), RSAHelper.getPrivateKeyFromHex(ownerPriKeyRsa));
        byte[] encrypt = RSAHelper.encrypt(decrypt, RSAHelper.getPublicKeyFromHex(memberPubKeyRsa));
